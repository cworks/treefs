...
public interface Glob_FromFs_FoldersOnly_FilesOnly<_ReturnType> {

    @MethodMeta(type = TransitionType.Recursive)
    Glob_FromFs_FoldersOnly_FilesOnly<_ReturnType> glob(String pattern);

    @MethodMeta(type = TransitionType.Lateral)
    Glob_FoldersOnly_FilesOnly<_ReturnType> fromFs(String fs);

    @MethodMeta(type = TransitionType.Lateral)
    Glob_FromFs_FilesOnly<_ReturnType> foldersOnly();

    @MethodMeta(type = TransitionType.Lateral)
    Glob_FromFs_FoldersOnly<_ReturnType> filesOnly();
