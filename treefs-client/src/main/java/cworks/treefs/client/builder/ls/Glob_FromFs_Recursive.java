...
public interface Glob_FromFs_Recursive<_ReturnType> {

    @MethodMeta(type = TransitionType.Recursive)
    Glob_FromFs_Recursive<_ReturnType> glob(String pattern);

    @MethodMeta(type = TransitionType.Lateral)
    Glob_Recursive<_ReturnType> fromFs(String fs);

    @MethodMeta(type = TransitionType.Lateral)
    Glob_FromFs<_ReturnType> recursive();
