...
#vertx runmod $MOD_NAME -conf $TREEFS_MOD/treefsconfig.json -cp build/mods/treefs-server\~treefs-server\~SNAPSHOT
...
while true; do
   if [ "x$LAUNCH_IN_BACKGROUND" = "x" ]; then
      eval vertx runmod $MOD_NAME -conf $TREEFS_MOD/treefsconfig.json -cp build/mods/treefs-server\~treefs-server\~SNAPSHOT
